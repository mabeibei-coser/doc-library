...
export function getDb() {
  ...
  // additive migration: 老库无 pinned 列时补上（置顶标记，列表排在最前）
  if (!docCols.has("pinned")) {
    _db.exec("ALTER TABLE documents ADD COLUMN pinned INTEGER NOT NULL DEFAULT 0");
  }
  // additive migration: 上传人姓名（admin-hub 创建时传入；历史数据为 NULL，前端显示 "—"）
  if (!docCols.has("uploaded_by_name")) {
    _db.exec("ALTER TABLE documents ADD COLUMN uploaded_by_name TEXT");
  }
