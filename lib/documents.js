...
export function createDocument(input) {
  ...
  const info = db
    .prepare(
      `INSERT INTO documents(title, category, subcategory, description, preview_json, attachment_json, required_tier, status, sort_order, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(
      input.title,
...
export function updateDocument(id, input) {
  ...
  const now = Date.now();
  db.prepare(
    `UPDATE documents SET title=?, category=?, subcategory=?, description=?, preview_json=?, attachment_json=?, required_tier=?, status=?, sort_order=?, updated_at=?
     WHERE id=?`
  ).run(
