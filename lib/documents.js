...
// 安防前台不展示的分类（属于其它业务域）。admin-hub 管理端（带 x-admin-secret）仍可见，
// 只在公开列表（publicScope=true）时隐藏。category 是自由文本，故用黑名单而非白名单——
// 保留历史无分类 / 其它安防分类（如"应急预案"）的文档照常展示，只挡掉这一域。
const PUBLIC_HIDDEN_CATEGORIES = ["人才ATA"];
 ...
 */
export function listDocuments({ category, subcategory, requiredTier, q, includeAll, publicScope } = {}) {
  const db = getDb();
  const where = [];
  const params = [];
  if (!includeAll) where.push("d.status = 'published'");
  if (publicScope && PUBLIC_HIDDEN_CATEGORIES.length) {
    const placeholders = PUBLIC_HIDDEN_CATEGORIES.map(() => "?").join(", ");
    where.push(`(d.category IS NULL OR d.category NOT IN (${placeholders}))`);
    params.push(...PUBLIC_HIDDEN_CATEGORIES);
  }
...
/** 全部分类（去重，给前台筛选 chip 用）。publicScope=true 时隐藏其它业务域分类。 */
export function listCategories({ publicScope } = {}) {
  const db = getDb();
